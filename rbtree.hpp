...
template<class T, class Compare>
ft::RedBlackTree<T> *tree_search_node(ft::RedBlackTree<T> *root, const T data_ref, Compare compare)
{
	if (!root || is_nil(root))
		return (NULL);
	if (is_equal(data_ref, *(root->_data), compare))
...
{
	ft::RedBlackTree<T> * node_to_delete = tree_search_node<T>(*root, data_ref, compare);
	if (!node_to_delete)
		return (NULL);
